...
u32 total_exits_across_all_vms = 0;
EXPORT_SYMBOL(total_exits_across_all_vms);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if (eax == 0x4fffffff) {
		// total number of exits that the hypervisor has processed across all VMs
		eax = total_exits_across_all_vms;
		printk(KERN_INFO "Total number of exits = %d", total_exits_across_all_vms);
	} else if (eax == 0x4ffffffe) {
		// store the high 32 bits of total time in ebx
		ebx = (total_time_for_all_exits >> 32);

		// store the low 32 bits of total time in ecx
